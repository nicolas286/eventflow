...
import type { buildPdfTickets } from "./db.ts";
...
function fitText(s: unknown, max = 80) {
...
export async function generateTicketsPdf(
  input: {
    ...
    currency: string;
    tickets: ReturnType<typeof buildPdfTickets>;
  },
) {
  ...
  function sanitizePdfText(value: unknown, targetFont = font) {
    const text = String(value ?? "");
    let result = "";

    for (const char of text) {
      try {
        targetFont.encodeText(char);
        result += char;
      } catch {
        // Caractère non supporté par WinAnsi :
        // emoji ou autre caractère Unicode non disponible
        // dans les polices PDF standards.
      }
    }
  ...
  function fitPdfText(value: unknown, max = 80, targetFont = font) {
    return fitText(
      sanitizePdfText(value, targetFont),
      max,
    );
  }
  ...
  function drawLabel(page: PDFPage, label: string, x: number, y: number) {
    page.drawText(
      sanitizePdfText(label, fontBold),
      {
        x,
        y,
        size: 8.5,
  ...
  function drawValue(
    ...
    const targetFont = bold ? fontBold : font;

    page.drawText(
      fitPdfText(value, 120, targetFont),
      {
        x,
        y,
        size,
  ...
  for (let i = 0; i < input.tickets.length; i += 2) {
    ...
    for (let slot = 0; slot < 2; slot++) {
      ...
      if (
        ticket.creates_attendees &&
        ticket.attendee_summary_lines.length > 0
      ) {
        cursorY = drawParticipantBox(
          page,
          ticket.attendee_summary_lines,
          leftX,
          cursorY,
          leftWidth,
        );
      ...
      if (
        input.startsAt &&
        cursorY - 28 >= contentBottomY
      ) {
        drawLabelValue(
          page,
          "Date",
          formatBrussels(input.startsAt) ??
            input.startsAt,
          leftX,
          cursorY,
          leftWidth,
      ...
      if (ticket.creates_attendees) {
        ...
        if (
          ...
          drawValue(
            page,
            "Aucun participant lié à ce billet.",
            leftX + 8,
            cursorY - 22,
            leftWidth - 16,
            9.2,
