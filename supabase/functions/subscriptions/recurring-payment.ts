import {
  type MollieWebhookResource,
  mollieWebhookResourceSchema,
  parseWebhookId,
} from "./webhook-contracts.ts";
import { BodyTooLargeError } from "../_shared/app/request-body.ts";
...
/* ---------------- Mollie helpers ---------------- */ async function mollieGet(
  mollieKey: string,
  url: string,
) {
  ...
  let data: MollieWebhookResource | null = null;
  try {
    const parsed = mollieWebhookResourceSchema.safeParse(
...
function listItems(listResp: MollieWebhookResource | null) {
...
function extractIdFromHref(href: unknown, prefix: string) {
  const s = typeof href === "string" ? href : "";
  const m = s.match(new RegExp(`(${prefix}_[a-zA-Z0-9]+)`));
  return m?.[1] ?? null;
}
function getPaymentIdsFromPay(pay: MollieWebhookResource | null) {
  const subscriptionId = pay?.subscriptionId ??
    extractIdFromHref(pay?._links?.subscription?.href, "sub") ?? null;
  const customerId = pay?.customerId ??
    extractIdFromHref(pay?._links?.customer?.href, "cst") ?? null;
  return {
    subscriptionId,
    customerId,
  };
}
function scorePayments(payments: MollieWebhookResource[]) {
  return payments.map((p) => {
    const status = String(p?.status ?? "").toLowerCase();
    const seq = String(p?.sequenceType ?? "").toLowerCase();
    const ts = (p?.paidAt ? Date.parse(String(p.paidAt)) : NaN) ||
      (p?.createdAt ? Date.parse(String(p.createdAt)) : NaN) || 0;
    return {
      p,
      status,
      seq,
...
/* ---------------- Handler ---------------- */ export async function handleRecurringPayment(
  req: Request,
): Promise<Response> {
  try {
    if (req.method !== "POST") {
    ...
    let paymentFromHook: MollieWebhookResource | null = null;
    if (hookId.startsWith("tr_")) {
      const payFetch = await mollieGet(
        mollieKey,
        `https://api.mollie.com/v2/payments/${hookId}`,
      );
      if (!payFetch.res.ok) {
    ...
    // -------------------------------
    const { data: srow, error: sErr } = await admin.from("subscriptions")
      .select("org_id, mollie_customer_id").eq(
    ...
    if (!subFetch.res.ok) {
      console.error(
        "[sub-webhook] fetch subscription failed -> retry",
        subFetch.rawText,
      );
      return json({
        ok: false,
    ...
    periodEnd = sub?.nextPaymentDate
      ? new Date(String(sub.nextPaymentDate)).toISOString()
      : null;
    ...
    if (applyErr) {
      console.error("[sub-webhook] apply_subscription_state failed", applyErr);
    }
    // -------------------------------
    ...
    if (paymentFromHook) {
      ...
      // déjà facturé ?
      const { data: already } = await admin.from("invoices").select("id").eq(
        "mollie_payment_id",
        pid,
      ).maybeSingle();
      if (already?.id) {
      ...
      const computedPeriodEnd = periodEnd ??
        (paymentFromHook?.paidAt
          ? new Date(String(paymentFromHook.paidAt)).toISOString()
          : null);
      const computedPeriodStart = computedPeriodEnd
        ? addMonthsISO(computedPeriodEnd, -1)
        : null;
      const { data: invoice, error: invErr } = await admin.rpc(
        "rpc_create_invoice_from_mollie_payment",
        {
          p_input: {
            org_id: orgId,
            ...
            paid_at: paymentFromHook?.paidAt
              ? new Date(String(paymentFromHook.paidAt)).toISOString()
      ...
      if (invoiceId) {
        await runInBackground(
          generateInvoicePdf(admin, invoiceId).catch(console.error),
        );
        await runInBackground(
          sendInvoiceToBillit(admin, invoiceId).catch(console.error),
        );
      }
      return json({
        ok: true,
    ...
    if (!payList.res.ok) {
      console.error(
        "[sub-webhook] cannot list subscription payments -> retry",
        payList.rawText,
      );
      return json({
        ok: false,
    ...
    const { data: existingInvoices, error: invReadErr } = await admin.from(
      "invoices",
    ).select("mollie_payment_id").in(
      "mollie_payment_id",
      ids.length ? ids : [
        "__none__",
      ],
    ...
    if (invReadErr) {
      console.error(
        "[sub-webhook] invoices lookup failed -> retry",
        invReadErr,
      );
      return json({
        ok: false,
    ...
    const invoicedSet = new Set(
      (existingInvoices ?? []).map((r) => String(r.mollie_payment_id)),
    );
    ...
    if (!candidate) {
      console.error(
        "[sub-webhook] no paid recurring payment not invoiced yet -> retry",
        {
          orgId,
          subscriptionId,
    ...
    const computedPeriodEnd = periodEnd ??
      (pay?.paidAt ? new Date(String(pay.paidAt)).toISOString() : null);
    const computedPeriodStart = computedPeriodEnd
      ? addMonthsISO(computedPeriodEnd, -1)
      : null;
    const { data: invoice, error: invErr } = await admin.rpc(
      "rpc_create_invoice_from_mollie_payment",
      {
        p_input: {
          ...
          paid_at: pay?.paidAt
            ? new Date(String(pay.paidAt)).toISOString()
            : new Date().toISOString(),
          period_start: computedPeriodStart,
          period_end: computedPeriodEnd,
          raw: pay,
